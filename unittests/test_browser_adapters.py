"""
Unit tests for browser adapters and factory.

Tests BrowserProtocol implementations (Selenium and Playwright adapters)
and BrowserFactory functionality.
"""

from unittest.mock import Mock

import pytest

from src.adapters.playwright_browser import PlaywrightBrowserAdapter
from src.adapters.selenium_browser import SeleniumBrowserAdapter
from src.core.browser_protocol import BrowserProtocol
from src.factories.browser_factory import (
    BrowserFactory,
    ChromeOptionsStrategy,
    EdgeOptionsStrategy,
    FirefoxOptionsStrategy,
)
from utils.exceptions import InvalidParameterException


class TestSeleniumBrowserAdapter:
    """Tests for SeleniumBrowserAdapter."""

    @pytest.fixture
    def mock_driver(self):
        """Create a mock Selenium WebDriver."""
        driver = Mock()
        driver.current_url = "https://example.com"
        driver.title = "Example Page"
        driver.page_source = "<html></html>"
        return driver

    @pytest.fixture
    def adapter(self, mock_driver):
        """Create SeleniumBrowserAdapter with mock driver."""
        return SeleniumBrowserAdapter(mock_driver, timeout=10)

    def test_implements_browser_protocol(self, adapter):
        """Test that adapter implements BrowserProtocol."""
        assert isinstance(adapter, BrowserProtocol)

    def test_timeout_property(self, adapter):
        """Test timeout getter and setter."""
        assert adapter.timeout == 10

        adapter.timeout = 15
        assert adapter.timeout == 15

    def test_navigate_valid_url(self, adapter, mock_driver):
        """Test navigating to a valid URL."""
        adapter.navigate("https://example.com")
        mock_driver.get.assert_called_once_with("https://example.com")

    def test_navigate_invalid_url_raises_exception(self, adapter):
        """Test navigating with invalid URL raises exception."""
        with pytest.raises(InvalidParameterException):
            adapter.navigate("")

        with pytest.raises(InvalidParameterException):
            adapter.navigate(None)

    def test_get_current_url(self, adapter, mock_driver):
        """Test getting current URL."""
        assert adapter.get_current_url() == "https://example.com"

    def test_get_title(self, adapter, mock_driver):
        """Test getting page title."""
        assert adapter.get_title() == "Example Page"

    def test_refresh(self, adapter, mock_driver):
        """Test page refresh."""
        adapter.refresh()
        mock_driver.refresh.assert_called_once()

    def test_get_page_source(self, adapter, mock_driver):
        """Test getting page source."""
        assert adapter.get_page_source() == "<html></html>"

    def test_execute_script_valid(self, adapter, mock_driver):
        """Test executing JavaScript."""
        mock_driver.execute_script.return_value = "result"

        result = adapter.execute_script("return document.title")
        assert result == "result"
        mock_driver.execute_script.assert_called_once()

    def test_execute_script_invalid_raises_exception(self, adapter):
        """Test executing invalid script raises exception."""
        with pytest.raises(InvalidParameterException):
            adapter.execute_script("")

        with pytest.raises(InvalidParameterException):
            adapter.execute_script(None)

    def test_take_screenshot(self, adapter, mock_driver):
        """Test taking screenshot."""
        mock_driver.get_screenshot_as_png.return_value = b"screenshot_data"

        result = adapter.take_screenshot("test.png")
        assert result == b"screenshot_data"
        mock_driver.save_screenshot.assert_called_once_with("test.png")

    def test_quit(self, adapter, mock_driver):
        """Test browser quit."""
        adapter.quit()
        mock_driver.quit.assert_called_once()

    def test_switch_to_default_content(self, adapter, mock_driver):
        """Test switching to default content."""
        adapter.switch_to_default_content()
        mock_driver.switch_to.default_content.assert_called_once()


class TestPlaywrightBrowserAdapter:
    """Tests for PlaywrightBrowserAdapter."""

    @pytest.fixture
    def mock_page(self):
        """Create a mock Playwright Page."""
        page = Mock()
        page.url = "https://example.com"
        page.title.return_value = "Example Page"
        page.content.return_value = "<html></html>"
        page.context = None  # No context for simple tests
        return page

    @pytest.fixture
    def adapter(self, mock_page):
        """Create PlaywrightBrowserAdapter with mock page."""
        return PlaywrightBrowserAdapter(mock_page, timeout=10)

    def test_implements_browser_protocol(self, adapter):
        """Test that adapter implements BrowserProtocol."""
        assert isinstance(adapter, BrowserProtocol)

    def test_timeout_property(self, adapter):
        """Test timeout getter and setter."""
        assert adapter.timeout == 10

        adapter.timeout = 15
        assert adapter.timeout == 15

    def test_navigate_valid_url(self, adapter, mock_page):
        """Test navigating to a valid URL."""
        adapter.navigate("https://example.com")
        mock_page.goto.assert_called_once_with("https://example.com")

    def test_navigate_invalid_url_raises_exception(self, adapter):
        """Test navigating with invalid URL raises exception."""
        with pytest.raises(InvalidParameterException):
            adapter.navigate("")

        with pytest.raises(InvalidParameterException):
            adapter.navigate(None)

    def test_get_current_url(self, adapter):
        """Test getting current URL."""
        assert adapter.get_current_url() == "https://example.com"

    def test_get_title(self, adapter, mock_page):
        """Test getting page title."""
        assert adapter.get_title() == "Example Page"

    def test_refresh(self, adapter, mock_page):
        """Test page refresh."""
        adapter.refresh()
        mock_page.reload.assert_called_once()

    def test_get_page_source(self, adapter, mock_page):
        """Test getting page source."""
        assert adapter.get_page_source() == "<html></html>"

    def test_execute_script_valid(self, adapter, mock_page):
        """Test executing JavaScript."""
        mock_page.evaluate.return_value = "result"

        result = adapter.execute_script("document.title")
        assert result == "result"
        mock_page.evaluate.assert_called_once()

    def test_execute_script_invalid_raises_exception(self, adapter):
        """Test executing invalid script raises exception."""
        with pytest.raises(InvalidParameterException):
            adapter.execute_script("")

        with pytest.raises(InvalidParameterException):
            adapter.execute_script(None)

    def test_take_screenshot(self, adapter, mock_page):
        """Test taking screenshot."""
        mock_page.screenshot.return_value = b"screenshot_data"

        result = adapter.take_screenshot("test.png")
        assert result == b"screenshot_data"
        mock_page.screenshot.assert_called_once_with(path="test.png")

    def test_quit(self, adapter, mock_page):
        """Test browser quit."""
        adapter.quit()
        mock_page.close.assert_called_once()


class TestBrowserFactory:
    """Tests for BrowserFactory."""

    def test_get_supported_browsers(self):
        """Test getting list of supported browsers."""
        browsers = BrowserFactory.get_supported_browsers()
        assert "chrome" in browsers
        assert "firefox" in browsers
        assert "edge" in browsers
        assert "chromium" in browsers

    def test_is_browser_supported(self):
        """Test checking if browser is supported."""
        assert BrowserFactory.is_browser_supported("chrome")
        assert BrowserFactory.is_browser_supported("firefox")
        assert BrowserFactory.is_browser_supported("edge")
        assert BrowserFactory.is_browser_supported("chromium")
        assert not BrowserFactory.is_browser_supported("safari")

    def test_register_strategy(self):
        """Test registering a new browser strategy."""
        # Create mock strategy
        mock_strategy = Mock()

        # Register strategy
        BrowserFactory.register_strategy("test_browser", mock_strategy)

        # Verify registration
        assert BrowserFactory.is_browser_supported("test_browser")

    def test_create_unsupported_framework_raises_error(self):
        """Test creating browser with unsupported framework raises error."""
        with pytest.raises(ValueError, match="Unsupported framework"):
            BrowserFactory.create(framework="unsupported")

    def test_create_unsupported_browser_raises_error(self):
        """Test creating unsupported browser raises error."""
        with pytest.raises(ValueError, match="Unsupported browser"):
            BrowserFactory.create(framework="selenium", browser="safari")


class TestBrowserOptionsStrategies:
    """Tests for browser options strategies."""

    def test_chrome_options_strategy_headless(self):
        """Test ChromeOptionsStrategy with headless mode."""
        strategy = ChromeOptionsStrategy()
        options = strategy.create_options(headless=True)

        # Verify headless argument is present
        assert "--headless=new" in options.arguments

    def test_chrome_options_strategy_non_headless(self):
        """Test ChromeOptionsStrategy without headless mode."""
        strategy = ChromeOptionsStrategy()
        options = strategy.create_options(headless=False)

        # Verify headless argument is not present
        assert "--headless=new" not in options.arguments

    def test_firefox_options_strategy_headless(self):
        """Test FirefoxOptionsStrategy with headless mode."""
        strategy = FirefoxOptionsStrategy()
        options = strategy.create_options(headless=True)

        # Verify headless argument is present
        assert "--headless" in options.arguments

    def test_edge_options_strategy_headless(self):
        """Test EdgeOptionsStrategy with headless mode."""
        strategy = EdgeOptionsStrategy()
        options = strategy.create_options(headless=True)

        # Verify headless argument is present
        assert "--headless" in options.arguments


class TestBrowserProtocolCompliance:
    """Tests to verify protocol compliance."""

    def test_selenium_adapter_has_all_protocol_methods(self):
        """Test SeleniumBrowserAdapter has all BrowserProtocol methods."""
        required_methods = [
            "navigate",
            "find_element",
            "find_elements",
            "execute_script",
            "get_current_url",
            "get_title",
            "refresh",
            "take_screenshot",
            "quit",
            "switch_to_frame",
            "switch_to_default_content",
            "get_page_source",
            "timeout",
        ]

        mock_driver = Mock()
        adapter = SeleniumBrowserAdapter(mock_driver)

        for method in required_methods:
            assert hasattr(adapter, method), f"Missing method: {method}"

    def test_playwright_adapter_has_all_protocol_methods(self):
        """Test PlaywrightBrowserAdapter has all BrowserProtocol methods."""
        required_methods = [
            "navigate",
            "find_element",
            "find_elements",
            "execute_script",
            "get_current_url",
            "get_title",
            "refresh",
            "take_screenshot",
            "quit",
            "switch_to_frame",
            "switch_to_default_content",
            "get_page_source",
            "timeout",
        ]

        mock_page = Mock()
        mock_page.url = "https://example.com"
        adapter = PlaywrightBrowserAdapter(mock_page)

        for method in required_methods:
            assert hasattr(adapter, method), f"Missing method: {method}"
